...
function fetchData(url) {
    fetch(url).then(function (rep) {
        ...
        if(data.next !== null){ //ou bien if(data.next)
            ...
            output.appendChild(btnNext);// !!!! POle mettre dans document.body crée à chaque fois un autre bouton Next
...
async function asyncData(url) {
    ...
    if(data.next){
        const btnNext = document.createElement("button");
        document.body.appendChild(btnNext); // le mettre dans document.body crée à chaque fois un autre bouton Next
        btnNext.textContent="Next";
        btnNext.addEventListener("click", function(){
            asyncData(data.next);
        })
